...
class AutogradTensor(AbstractTensor):
    ...
    @overloaded.method
    def pad(self, pad, mode, value):
        return torch.nn.functional.pad(self, pad=pad, mode=mode, value=value)

    # @overloaded.m/ethod
    def conv2d_input_grad(self, self_shape, weight, grad, *params):
        return conv2dGrad.conv2d_input(self_shape, weight, grad, *params)

    def conv2d_weight_grad(self, weight_shape, grad, *params):
        return conv2dGrad.conv2d_weight(self, weight_shape, grad, *params)
    
    # def size(self, dim):
    #     return self.child.child.size(dim)
    ...
    def __getattribute__(self, name):
        ...
        if grad_fn is not None:
            ...
            def method_with_grad(*args, **kwargs):
                ...
                if name == 'pad':
                    result = torch.nn.functional.pad(new_self, **kwargs) 
                elif name == 'conv2d':
                    result = torch.nn.functional.conv2d(new_self, *new_args)
                elif name == 'max_pool2d':
                    result = torch.nn.functional.max_pool2d(new_self, *new_args)
                elif name == 'flatten':
                    result = torch.flatten(new_self, **new_kwargs)
                elif name == 'softmax':
                    new_self_normalized = new_self - new_self.max(dim = 1, keepdim = True)[0]
                    new_self_normalized_exp = new_self_normalized.exp()
                    result = new_self_normalized_exp/new_self_normalized_exp.sum(dim = 1, keepdim = True)
                else:
                    result = getattr(new_self, name)(*new_args, **new_kwargs)
                
                # Put back SyftTensor on the tensors found in the response
                if name == 'max':
                    result1, result2 = hook_args.hook_response(name, result, wrap_type=type(self))
                    result1.grad_fn = grad_fn(self, *args, **kwargs)
    ...
    def torch(module):
        ...
        def size(input, dim):
            return input.child.child.size(dim)
        ...
        def nn(module):
            ...
            def functional(module):
                ...
                def conv2d(*args): ##Newly added for conv2d
                    return  args[0].conv2d(*args[1:])
                ...
                def max_pool2d(*args): ##Newly added for max_pool2d
                    return args[0].max_pool2d(*args[1:])
