...
class FemnistNetSmall(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 16, kernel_size=5, stride=1,
                               padding=0)  ##output shape (batch, 32, 28, 28)self.pool1 = nn.MaxPool2d(2, stride=2) ## output shape (batch, 32, 14, 14)
        self.pool1 = nn.MaxPool2d(2, stride=2)  ## output shape (batch, 64, 7, 7)

        self.conv2 = nn.Conv2d(16, 32, kernel_size=5, stride=1, padding=0)  ##output shape (batch, 64, 14, 14)
        self.pool2 = nn.MaxPool2d(2, stride=2)  ## output shape (batch, 64, 7, 7)

        self.fc1 = nn.Linear(2592, 1024)  ##input of [BatchSize, 3136]. output of [BatchSize, 2048]
        self.fc2 = nn.Linear(1024, 62)  ##input of [BatchSize, 2048]. output of [BatchSize, 62]
    ...
    def forward(self, x):
        ...
        x = self.pool2(x)
        print("self.pool2(x).shape", self.pool2(x).shape)
        x = x.flatten(start_dim=1)
        print("xflatten.shape", x.shape)
        x = self.fc1(x)
        l1_activations = th.nn.functional.relu(x)
...
print(train_X[:1].numpy().astype(int))
print(train_y[:1].numpy())
model_small.forward(train_X[:1])[0].detach().numpy()
...
loss, acc, logits, target, *updated_params, _ = training_plan_small.build(train_X[0:20], train_y[0:20], batch_size, lr, model_params_small, trace_autograd=True)
...
accuracy, loss = evaluate_model_plan.torchscript(X_test, y_test, len_y_test, model_params_small)#evaluate_op(X_test, y_test, len_y_test, model_params)
...
import math
for e in range(1):
    epoch_acc, epoch_loss = evaluate_model_plan.torchscript(X_test, y_test, len_y_test, model_params_small)
    ...
    for b in range(totalBatches):
        ...
        loss, acc, logits, avg_act_over_examples, *updated_params = training_plan_small.torchscript(x_b, y_b, batch_size, lr, model_params_small)#forward_op(x_b, y_b, batch_size, lr, model_params)
