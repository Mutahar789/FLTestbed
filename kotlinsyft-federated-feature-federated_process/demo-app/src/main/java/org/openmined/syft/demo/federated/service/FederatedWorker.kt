...
//        val baseUrl = inputData.getString(BASE_URL) ?: return Result.failure()
        ...
        val config = SyftConfiguration.builder(serviceContext, "192.168.100.30:5000")
                .enableBackgroundServiceExecution()
                .setCacheTimeout(0L)
                .build()
        setForegroundAsync(createForegroundInfo("0"))

        val localFEMNISTDataSource = LocalFEMNISTDataSource(serviceContext.resources)
        val femnistDataRepository = FEMNISTDataRepository(localFEMNISTDataSource)
        val trainingTask = TrainingTask(config, BuildConfig.SYFT_AUTH_TOKEN, femnistDataRepository, DATASET.FEMNIST)


        return awaitTask(trainingTask)
    }

    private suspend fun awaitTask(task: TrainingTask) = suspendCoroutine<Result> { continuation ->
        Log.d("FederatedWorker", "awaitTAsk")
