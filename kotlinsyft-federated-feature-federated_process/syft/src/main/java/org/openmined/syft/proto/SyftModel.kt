...
data class SyftModel(
    ...
    var modelDownloadStartTime = System.currentTimeMillis()
    var modelDownloadEndTime = System.currentTimeMillis()
    ...
    fun createWeightedDiff(oldSyftState: SyftState, diffScriptLocation: String, weight: Int): SyftState {
        return modelSyftState?.createWeightedDiff(oldSyftState, diffScriptLocation, weight)
               ?: throw IllegalStateException("Model parameters not downloaded yet")
    }
    ...
    fun applyWeightage(weight: Int) {
        modelSyftState?.updateWeightedState(weight)
    }

    fun setModelDownloadStartTime() {
        modelDownloadStartTime = System.currentTimeMillis()
    }

    fun setModelDownloadEndTime() {
        modelDownloadEndTime = System.currentTimeMillis()
    }

    fun getModelDownloadTime() = modelDownloadEndTime - modelDownloadStartTime
}
