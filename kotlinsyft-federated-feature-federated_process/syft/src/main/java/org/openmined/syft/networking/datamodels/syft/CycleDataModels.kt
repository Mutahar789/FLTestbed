...
internal data class CycleRequest(
    ...
    @SerialName("cycle_start_request_key")
    val cycleStartRequestKey: String,
...
//    jsonConfig.str
    ...
    override fun deserialize(decoder: Decoder): CycleResponseData {
        ...
        Log.v("CycleDataModels", "========================== >>> data  " + data)
        Log.v("CycleDataModels", "========================== >>> response  " + response)

        return if (response.getObject("data").getPrimitive("status").content == CYCLE_ACCEPT)
            json.parse(
    ...
    override fun serialize(encoder: Encoder, obj: CycleResponseData) {
        ...
        when (obj) {
            is CycleResponseData.CycleAccept -> {
                val accept = json.toJson(CycleResponseData.CycleAccept.serializer(),obj)
                Log.v("CycleDataModels", "=================================>>> accept >>> " + accept)
