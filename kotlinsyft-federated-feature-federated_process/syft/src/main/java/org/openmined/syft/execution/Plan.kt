...
class Plan(val job: SyftJob, val planId: String, val planName: String) {
    ...
    fun execute(vararg iValues: IValue): IValue? {
        ...
        if (localModuleState == null) {
            Log.e(TAG, "pytorch module not initialized yet, $planId $planName")
            return null
        }
...
//        if (Syft.plansMap.containsKey(torchScriptLocation))
//            Log.v("Plan", "======================================= Plans already loaded ${Syft.plansMap}")
//            return

        pyTorchModule = Module.load(torchScriptLocation)
//        Syft.plansMap.put(torchScriptLocation, true)
    }
}