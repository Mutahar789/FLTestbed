...
internal class SocketClient(
    ...
    private val timeout: UInt = 1000000u,
    ...
    override fun saveFirebaseDeviceToken(tokenRequest: TokenRequest): Single<TokenResponseData> {
        syftWebSocket.send(serializeNetworkModel(REQUESTS.TOKEN_FCM_SAVE, tokenRequest))
        return messageProcessor.onBackpressureDrop()
                .ofType(TokenResponseData::class.java)
                .firstOrError()
    }

    override fun updateWorkerOnlineStatus(tokenRequest: WorkerStatusRequest): Single<TokenResponseData> {
        TODO("Not yet implemented")
    }

    override fun uploadMetrics(metricsRequest: TrainingStatsRequestList): Single<TokenResponseData> {
        TODO("Not yet implemented")
    }

    override fun reportModel(reportRequest: ReportRequest): Single<ReportResponseData> {
        connectWebSocket()
        syftWebSocket.send(serializeNetworkModel(REQUESTS.REPORT, reportRequest))
        return messageProcessor.onBackpressureDrop()
                .ofType(ReportResponseData::class.java)
                .firstOrError()
    }

    override fun reportStats(reportRequest: ReportStatRequest): Single<ReportStatResponseData> {
        TODO("Not yet implemented")
    }
